...
/obj/item/slimecross/warping/interact_with_atom(turf/open/floor/target, mob/living/user, list/modifiers)
...
/obj/effect/warped_rune/greyspace/Initialize(mapload)
	. = ..()
...
/obj/effect/warped_rune/greyspace/do_effect(mob/user)
	for(var/obj/item/slime_extract/extract in rune_turf)
		if(extract.type == extracttype || !extracttype) //check if the extract is the first one or of the right color.
...
/obj/effect/warped_rune/darkpurplespace/Initialize(mapload)
	. = ..()
	if(isnull(materials))
		materials = list(
...
/obj/effect/warped_rune/bluespace/do_effect(mob/user)
	if(isnull(blue_storage)) // Why isn't this on init? Linters of course!
...
/obj/effect/warped_rune/redspace/on_entered(datum/source, atom/movable/AM, oldloc)
	if(ishuman(AM))
		...
		H.add_blood_DNA(list("Non-human DNA" = random_human_blood_type()))
...
/obj/effect/warped_rune/pinkspace/on_entered(datum/source, mob/living/carbon/human, oldloc)
	if(istype(human))
		human.add_mood_event("jolly", /datum/mood_event/jolly)
		playsound(rune_turf, "sound/weapons/thudswoosh.ogg", 50, TRUE)
		to_chat(human, span_notice("You feel happier."))
		activated_on_step = TRUE
	return ..()
...
//		/obj/item/clothing/suit/space/hardsuit/ancient, I swear this exists somewhere, but i couldn't find it
...
/obj/effect/warped_rune/blackspace // This used to give clockwork enchantments, now its a whetstone. Society.
...
/obj/effect/warped_rune/blackspace/attackby(obj/item/I, mob/living/user, params)
	if(I.force > 30)
		to_chat(user, span_warning("[I] resists the attempt to enchant it!"))
		return
	if(I.force > initial(I.force)) // No upgrading stuff twice
		to_chat(user, span_warning("[I] has already been refined before. It cannot be enchanted further!"))
		return
	to_chat(user, span_brass("You begin placing [I] onto [src]."))
	if(do_after(user, 6 SECONDS, target = I))
		if(I.force > initial(I.force)) // i love do_afters
			to_chat(user, span_warning("[I] has already been refined before. It cannot be enchanted further!"))
			return
...
/obj/effect/warped_rune/blackspace/proc/upgrade_weapon(obj/item/I, mob/living/user)
	if(I.force > initial(I.force)) // No sharpening stuff twice
		to_chat(user, span_warning("[I] has already been refined before. It cannot be enchanted further!"))
		return
